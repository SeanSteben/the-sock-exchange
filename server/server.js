...
app.get('/socks/search', async (req, res) => {
    const searchedColor = req.body.searchTerm; 
});
...
app.post('/socks/search', async (req, res) => {
    try {
        ...
        const socks = await collection.find({"sockDetails.color" : searchColor} ).toArray();
        res.json(socks);
...
app.delete('/socks/:id', async (req, res) => {
    try {
        ...
        const socks = await collection.deleteOne({_id : new ObjectId(id)});
